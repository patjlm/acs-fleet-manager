/*
 * Red Hat Advanced Cluster Security Service Fleet Manager
 *
 * Red Hat Advanced Cluster Security Service Fleet Manager APIs that are used by internal services e.g fleetshard operators.
 *
 * API version: 1.4.0
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package private

// DataPlaneCentralStatus Schema of the status object for a Central component
type DataPlaneCentralStatus struct {
	// The status conditions of a Central component
	Conditions []DataPlaneClusterUpdateStatusRequestConditions `json:"conditions,omitempty"`
	Versions   DataPlaneCentralStatusVersions                  `json:"versions,omitempty"`
	// Routes created for a Central component
	Routes *[]DataPlaneCentralStatusRoutes `json:"routes,omitempty"`
}
